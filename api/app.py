...
with tab_buscar:
    # Estado de paginación persistente entre reruns
    ...
    if params or submitted:
        ...
        if rows:
            ...
            display_df = pd.DataFrame({
                "Número":          df["numero"],
                "Fecha":           df["fecha"],
                "PDF":             df["pdf_url"].fillna(""),
                "Sentido":         df["sentido"].fillna(""),
                "Ley":             df["ley_impugnada"].fillna(""),
                "Tipo de recurso": df["tipo_recurso"].fillna(""),
                "Descripción":     df["descripcion"].fillna("").str[:220],
            })

            st.dataframe(
                display_df,
                use_container_width=True,
                height=min(60 + len(rows) * 38, 600),
                column_config={
                    "Número":          st.column_config.TextColumn("Número"),
                    "Fecha":           st.column_config.TextColumn("Fecha",           width="small"),
                    "PDF":             st.column_config.LinkColumn("PDF", display_text="Ver PDF", help="Haz clic para abrir el PDF"),
                    "Sentido":         st.column_config.TextColumn("Sentido",         width="small"),
                    "Ley":             st.column_config.TextColumn("Ley",             width="small"),
                    "Tipo de recurso": st.column_config.TextColumn("Tipo de recurso", width="medium"),
